	...
	public:
		...
		bool process_wndproc_message( UINT msg, WPARAM wparam, LPARAM lparam ) noexcept
		{
			...
			case WM_XBUTTONDOWN:
			{
				...
				if ( button == XBUTTON1 )
				{
					this->m_current.push_key_press( VK_XBUTTON1 );
					this->m_current.key_down_map[ VK_XBUTTON1 ] = true;
				}
				else if ( button == XBUTTON2 )
				{
					this->m_current.push_key_press( VK_XBUTTON2 );
					this->m_current.key_down_map[ VK_XBUTTON2 ] = true;
				}
			...
			case WM_XBUTTONUP:
			{
				...
				if ( button == XBUTTON1 )
				{
					this->m_current.push_key_release( VK_XBUTTON1 );
					this->m_current.key_down_map[ VK_XBUTTON1 ] = false;
				}
				else if ( button == XBUTTON2 )
				{
					this->m_current.push_key_release( VK_XBUTTON2 );
					this->m_current.key_down_map[ VK_XBUTTON2 ] = false;
				}
		...
		void render( const zui::style& style, const input_state& input ) override
		{
			...
			for ( int i = 0; i < static_cast< int >( this->m_items.size( ) ); ++i )
			{
				...
				if ( hover_anim > 0.01f )
				{
					...
					dl.add_rect_filled_multi_color( item_rect.x, item_rect.y + slide_offset, item_rect.w, item_rect.h, hover_left, hover_right, hover_right, hover_left );
				}
				...
				dl.add_text( text_x, text_y, this->m_items[ i ].c_str( ), text_col );
			}
	...
	public:
		...
		void render( const zui::style& style, const input_state& input ) override
		{
...

			{
				...
				dl.add_text( text_x, text_y, "copy", text_col );
			}

			{
				...
				dl.add_text( text_x, text_y, "paste", text_col );
			}
		}
...
namespace zui {
	...
	namespace detail {
		...
		float mouse_delta_x( ) { return ctx( ).input( ).mouse_delta_x( ); }
		float mouse_delta_y( ) { return ctx( ).input( ).mouse_delta_y( ); }
		float scroll_delta( ) { return ctx( ).input( ).scroll_delta( ); }

		bool right_mouse_down( ) { return ctx( ).input( ).right_mouse_down( ); }
		bool right_mouse_clicked( ) { return ctx( ).input( ).right_mouse_clicked( ); }
		bool right_mouse_released( ) { return ctx( ).input( ).right_mouse_released( ); }

		bool is_key_down( int vk )
		{
			const auto& map = ctx( ).input( ).current( ).key_down_map;
			const auto it = map.find( vk );
			return it != map.end( ) && it->second;
		}

		std::span<const int> key_presses( ) { return ctx( ).input( ).current( ).key_presses( ); }
		std::span<const int> key_releases( ) { return ctx( ).input( ).current( ).key_releases( ); }
		std::span<const wchar_t> input_chars( ) { return ctx( ).input( ).current( ).chars( ); }
	...
	bool begin_window( std::string_view title, float& x, float& y, float& w, float& h, bool resizable, float min_w, float min_h )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled_multi_color( abs.x, abs.y, abs.w, abs.h, col_tl, base_color, col_br, base_color );

		if ( style.border_thickness > 0.0f )
		{
			zdraw::get_draw_list( ).add_rect( abs.x, abs.y, abs.w, abs.h, style.window_border, style.border_thickness );
		}

		zdraw::get_draw_list( ).push_clip_rect( abs.x, abs.y, abs.right( ), abs.bottom( ) );
		ctx( ).push_id( id );
	...
	{
		const auto win = ctx( ).current_window( );
		zdraw::get_draw_list( ).pop_clip_rect( );
	...
	bool begin_nested_window( std::string_view title, float w, float h )
	{
		...
		if ( style.border_thickness > 0.0f )
		{
			zdraw::get_draw_list( ).add_rect( abs.x, abs.y, abs.w, abs.h, style.nested_border, style.border_thickness );
		}
	...
	bool begin_group_box( std::string_view title, float w, float h )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled( abs.x, border_y, abs.w, box_height, style.group_box_bg );
		zdraw::get_draw_list( ).add_rect( abs.x, border_y, abs.w, box_height, style.group_box_border, style.border_thickness );
		...
		if ( !title.empty( ) )
		{
			...
			zdraw::get_draw_list( ).add_rect_filled( gap_start, abs.y, gap_end - gap_start, title_h, style.group_box_bg );
			zdraw::get_draw_list( ).add_rect( gap_start, abs.y, gap_end - gap_start, title_h, style.group_box_border, style.border_thickness );

			const auto text_y = abs.y + ( title_h - title_h_measured ) * 0.5f;
			zdraw::get_draw_list( ).add_text( text_x, text_y, title_str, style.group_box_title_text );
		}
		...
		ctx( ).push_window( std::move( state ) );
		zdraw::get_draw_list( ).push_clip_rect( abs.x, abs.y + title_h, abs.right( ), abs.bottom( ) );
		ctx( ).push_id( id );
	...
	void separator( )
	{
		...
		zdraw::get_draw_list( ).add_line( abs.x, abs.y + sep_pad, abs.x + avail_w, abs.y + sep_pad, sep_col, sep_h );
	}
	...
	void text( std::string_view label )
	{
		...
		zdraw::get_draw_list( ).add_text( abs.x, abs.y, label, ctx( ).get_style( ).text );
	}
	...
	void text_colored( std::string_view label, const zdraw::rgba& color )
	{
		...
		zdraw::get_draw_list( ).add_text( abs.x, abs.y, label, color );
	}
	...
	void text_gradient( std::string_view label, const zdraw::rgba& color_left, const zdraw::rgba& color_right )
	{
		...
		zdraw::get_draw_list( ).add_text_multi_color( abs.x, abs.y, label, color_left, color_right, color_right, color_left );
	}
	...
	void text_gradient_vertical( std::string_view label, const zdraw::rgba& color_top, const zdraw::rgba& color_bottom )
	{
		...
		zdraw::get_draw_list( ).add_text_multi_color( abs.x, abs.y, label, color_top, color_top, color_bottom, color_bottom );
	}
	...
	void text_gradient_four( std::string_view label, const zdraw::rgba& color_tl, const zdraw::rgba& color_tr, const zdraw::rgba& color_br, const zdraw::rgba& color_bl )
	{
		...
		zdraw::get_draw_list( ).add_text_multi_color( abs.x, abs.y, label, color_tl, color_tr, color_br, color_bl );
	}
	...
	bool button( std::string_view label, float w, float h )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled_multi_color( abs.x, abs.y, abs.w, abs.h, col_top, col_top, col_bottom, col_bottom );
		zdraw::get_draw_list( ).add_rect( abs.x, abs.y, abs.w, abs.h, border_col );
		...
		if ( !display_label.empty( ) )
		{
			...
			zdraw::get_draw_list( ).add_text( text_x, text_y, label_text, style.text );
		}
	...
	bool checkbox( std::string_view label, bool& v )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled( abs.x, abs.y, abs.w, abs.h, style.checkbox_bg );
		zdraw::get_draw_list( ).add_rect( abs.x, abs.y, abs.w, abs.h, border_col );
		...
		if ( ease_t > 0.01f )
		{
			...
			zdraw::get_draw_list( ).add_rect_filled_multi_color( fill_x, fill_y, scaled_w, scaled_h, col_top, col_top, col_bottom, col_bottom );
			...
			if ( ease_t < 0.4f && ease_t > 0.0f )
			{
				...
				zdraw::get_draw_list( ).add_rect( abs.x - ring_expand, abs.y - ring_expand, abs.w + ring_expand * 2.0f, abs.h + ring_expand * 2.0f, ring_col, 1.5f );
			}
		}
	...
	namespace
	{
		...
		bool slider_impl( std::string_view label, T& v, T v_min, T v_max, std::string_view format )
		{
			...
			if ( !display_label.empty( ) )
			{
				...
				zdraw::get_draw_list( ).add_text( abs.x, abs.y, label_text, style.text );
			}

			const auto value_col = lerp( style.text, lighten( style.slider_fill, 1.2f ), hover_anim * 0.35f );
			zdraw::get_draw_list( ).add_text( abs.x + slider_width - value_w, abs.y, value_buf, value_col );

			const auto track_bg = darken( style.slider_bg, 0.7f );
			zdraw::get_draw_list( ).add_rect_filled( track_rect.x, track_rect.y, track_rect.w, track_rect.h, track_bg );

			const auto shadow_col = zdraw::rgba{ 0, 0, 0, static_cast< std::uint8_t >( 40 + hover_anim * 10 ) };
			zdraw::get_draw_list( ).add_rect_filled_multi_color( track_rect.x + 1.0f, track_rect.y + 1.0f, track_rect.w - 2.0f, track_rect.h * 0.4f, shadow_col, shadow_col, alpha( shadow_col, 0.0f ), alpha( shadow_col, 0.0f ) );

			const auto border_col = lerp( style.slider_border, lighten( style.slider_fill, 0.6f ), hover_anim * 0.25f );
			zdraw::get_draw_list( ).add_rect( track_rect.x, track_rect.y, track_rect.w, track_rect.h, border_col );
			...
			if ( fill_w > 0.5f )
			{
				...
				zdraw::get_draw_list( ).add_rect_filled_multi_color( fill_x, fill_y, fill_w, fill_h, fill_left, fill_right, fill_right, fill_left );

				const auto shine = zdraw::rgba{ 255, 255, 255, static_cast< std::uint8_t >( 20 + hover_anim * 15 ) };
				zdraw::get_draw_list( ).add_rect_filled_multi_color( fill_x, fill_y, fill_w, fill_h * 0.4f, shine, shine, alpha( shine, 0.0f ), alpha( shine, 0.0f ) );
			}
			...
			zdraw::get_draw_list( ).add_rect_filled( knob_x + 1.0f, knob_y + knob_shadow_offset, knob_width, knob_height, knob_shadow );

			const auto knob_bg = lerp( style.slider_bg, lighten( style.slider_bg, 1.2f ), hover_anim * 0.4f );
			zdraw::get_draw_list( ).add_rect_filled( knob_x, knob_y, knob_width, knob_height, knob_bg );

			const auto knob_highlight = zdraw::rgba{ 255, 255, 255, static_cast< std::uint8_t >( 15 + hover_anim * 20 ) };
			zdraw::get_draw_list( ).add_rect_filled_multi_color( knob_x + 1.0f, knob_y + 1.0f, knob_width - 2.0f, knob_height * 0.35f, knob_highlight, knob_highlight, alpha( knob_highlight, 0.0f ), alpha( knob_highlight, 0.0f ) );

			const auto knob_border = lerp( style.slider_border, lighten( style.slider_fill, 0.8f ), hover_anim * 0.5f );
			zdraw::get_draw_list( ).add_rect( knob_x, knob_y, knob_width, knob_height, knob_border );
	...
	bool keybind( std::string_view label, int& key )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled( button_rect.x, button_rect.y, button_rect.w, button_rect.h, bg_col );
		...
		if ( wait_anim > 0.01f )
		{
			...
			if ( shift > 0.5f )
			{
				zdraw::get_draw_list( ).add_rect_filled_multi_color( fill_x, fill_y, fill_w, fill_h, col_right, col_left, col_left, col_right );
			}
			else
			{
				zdraw::get_draw_list( ).add_rect_filled_multi_color( fill_x, fill_y, fill_w, fill_h, col_left, col_right, col_right, col_left );
			}
		...
		zdraw::get_draw_list( ).add_rect( button_rect.x, button_rect.y, button_rect.w, button_rect.h, border_col );
		...
		zdraw::get_draw_list( ).add_text( text_x, text_y, button_text, style.text );
		...
		if ( !display_label.empty( ) )
		{
			...
			zdraw::get_draw_list( ).add_text( label_x, label_y, label_text, style.text );
		}
	...
	bool combo( std::string_view label, int& current_item, const char* const items[ ], int items_count, float width )
	{
		...
		zdraw::get_draw_list( ).add_text( text_x, text_y, current_text, style.text );
		...
		if ( is_open )
		{
			...
			zdraw::get_draw_list( ).add_polyline( points, arrow_col, false, 1.5f );
		}
		else
		{
	...
	bool multicombo( std::string_view label, bool* selected_items, const char* const items[ ], int items_count, float width )
	{
		...
		zdraw::get_draw_list( ).push_clip_rect( clip_left, clip_top, clip_right, clip_bottom );
		...
		if ( needs_scroll && scroll_state.scroll_offset > 0.01f )
		{
			...
			zdraw::get_draw_list( ).add_text( text_x + offset1, text_y, display_text, style.text );

			const auto offset2 = offset1 + total_width;
			if ( offset2 < max_text_width )
			{
				zdraw::get_draw_list( ).add_text( text_x + offset2, text_y, display_text, style.text );
			}
		}
		else if ( needs_scroll )
		{
			...
			truncated += "...";
			zdraw::get_draw_list( ).add_text( text_x, text_y, truncated, style.text );
		}
		else
		{
			zdraw::get_draw_list( ).add_text( text_x, text_y, display_text, style.text );
		}
	...
	bool color_picker( std::string_view label, zdraw::rgba& color, float width, bool show_alpha )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled( swatch_rect.x, swatch_rect.y, swatch_rect.w, swatch_rect.h, style.color_picker_bg );

		constexpr auto pad{ 2.0f };
		zdraw::get_draw_list( ).add_rect_filled( swatch_rect.x + pad, swatch_rect.y + pad, swatch_rect.w - pad * 2.0f, swatch_rect.h - pad * 2.0f, color );
		zdraw::get_draw_list( ).add_rect( swatch_rect.x, swatch_rect.y, swatch_rect.w, swatch_rect.h, border_col );
	...
	bool text_input( std::string_view label, std::string& text, std::size_t max_length, std::string_view hint )
	{
		...
		zdraw::get_draw_list( ).add_rect_filled( frame_rect.x, frame_rect.y, frame_rect.w, frame_rect.h, bg_col );
		zdraw::get_draw_list( ).add_rect( frame_rect.x, frame_rect.y, frame_rect.w, frame_rect.h, border_col );
		...
		zdraw::get_draw_list( ).push_clip_rect( frame_rect.x + text_padding_x, frame_rect.y, frame_rect.x + text_padding_x + text_area_w, frame_rect.y + frame_rect.h );
		...
		if ( selection_anim > 0.01f )
		{
			...
			sel_col.a = alpha;
			zdraw::get_draw_list( ).add_rect_filled( sel_x, text_y - 1.0f + offset_y, sel_w, scaled_h, sel_col );
		}
		...
		if ( text.empty( ) && !is_active && !hint.empty( ) )
		{
			...
			hint_col.a = 100;
			zdraw::get_draw_list( ).add_text( frame_rect.x + text_padding_x, text_y, hint, hint_col );
		}
		else if ( !text.empty( ) )
		{
			zdraw::get_draw_list( ).add_text( text_x, text_y, text, style.text );
		}
		...
		if ( is_active )
		{
			...
			zdraw::get_draw_list( ).add_rect_filled( state.cursor_anim_x, cursor_y, 1.0f, cursor_h, cursor_col );
		}
		else
		{
