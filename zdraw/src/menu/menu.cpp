...
namespace menu {
	...
	void draw( )
	{
...

		{
			...
			if ( zui::begin_window( "model", win_x, win_y, win_w, win_h, true, 300.0f, 300.0f ) )
			{
...

		{
			...
			if ( zui::begin_window( "widgets", win2_x, win2_y, win2_w, win2_h, true, 350.0f, 538.0f ) )
			{
				auto [avail_w, avail_h] = zui::get_content_region_avail( );
				...
				if ( zui::begin_group_box( "checkboxes & sliders", avail_w ) )
				{
					...
					static float slider1{ 0.25f };
					static int slider2{ 75 };
				...
				if ( zui::begin_group_box( "combos & colors", avail_w ) )
				{
					...
					static auto color{ zdraw::rgba( 160, 170, 220, 255 ) };
